...
class LogbookController extends Controller
{
    public function index(){
        ...
        $logbook = [
            'total' => $logbook->count(),
            'pending' => $logbook->where('status', 2)->count(),
            'diterima' => $logbook->where('status', 1)->count(),
            'ditolak' => $logbook->where('status', 0)->count(),
        ]; 
        return view('admin.logbook', compact('logbook'));
    }
       
    public function show(Request $request)  {
        
        $loogbook = LogBook::with('nimmhs');
        ...
        return DataTables::of($loogbook)
            ...
            ->addColumn('action', function ($row) {
                ...
                $btn = "<a class='btn-icon btn-approve' data-status='{$row->status}' data-url='approve/$row->id_loogbook' data-id='{$row->id_loogbook}'>
                            <i class='ti ti-file-check text-success'></i>
                        </a>
                        <a class='btn-icon btn-reject' data-status='{$row->status}' data-url='tolak/$row->id_loogbook' data-id='{$row->id_loogbook}'>
    ...
    public function approve(String $id) {
        try {
            $data = LogBook::where('id_loogbook', $id)->first();
            $data->status = 1; 
            $data->save();
            // dd($data);
